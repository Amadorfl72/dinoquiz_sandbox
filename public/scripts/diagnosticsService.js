...
/**
 * Local diagnostics service (TRIOFSND-317/318, PRD "Diagnóstico y métricas
 ...
 * documented for public/scripts/homeScreen.js/modeStorage.js. Every real
 * caller (public/scripts/roundDiagnosticsService.js, modeSelectorScreen.js,
 * gameFlow.js, main.js) resolves this service via `window.DinoQuiz.services.
 * diagnostics` first and `require('../../src/services/diagnostics')` as its
 * Node/Jest fallback -- before this file existed, the browser had no `<script>`
 * for this service at all, so that first branch always missed and every one
 * of those counters/errors silently no-op'd in the real, unbundled PWA. This
 * registers on `window.DinoQuiz.services.diagnostics` (same dual CommonJS/
 * browser-global pattern as modeStorage.js/roundDiagnosticsService.js) so
 * that branch now actually resolves; the canonical `src/services/
 * diagnostics.js` re-exports this file for Node/Jest and other `src/` modules.
 *
...
(function () {
  var COUNTERS_KEY = 'dinoquiz:metrics:counters';
  var ERRORS_KEY = 'dinoquiz:diagnostics:errors';
  var RETENTION_KEY = 'dinoquiz:diagnostics:retention';
  ...
  var DIAGNOSTICS_KEYS = [COUNTERS_KEY, ERRORS_KEY, RETENTION_KEY];

  var MAX_ERROR_ENTRIES = 500;
  var RETENTION_WINDOW_DAYS = 7;
  ...
  function readJSON(key, fallbackValue, isValid, storageAdapter) {
    var raw = readRaw(key, storageAdapter);
    if (raw === null) {
  ...
  function daysBetween(fromDateString, toDateString) {
    var from = new Date(fromDateString + 'T00:00:00');
    var to = new Date(toDateString + 'T00:00:00');
  ...
  function incrementCounter(name, storageAdapter) {
    ...
    var counters = readJSON(COUNTERS_KEY, {}, isCountersMap, storageAdapter);
    var next = (counters[name] || 0) + 1;
    counters[name] = next;
  ...
  function recordLocalReturn(date, storageAdapter) {
    var today = localDateString(date);
    var record = readJSON(RETENTION_KEY, { installDate: today, returnDates: [] }, isRetentionRecord, storageAdapter);

    if (record.returnDates.indexOf(today) === -1) {
      record.returnDates = record.returnDates.concat([today]);
    }
  ...
  function computeSevenDayRetention(storageAdapter) {
    var record = readJSON(RETENTION_KEY, null, isRetentionRecord, storageAdapter);
    if (!record) {
      return false;
    }
    return record.returnDates.some(function (returnDate) {
      var diff = daysBetween(record.installDate, returnDate);
  ...
  function recordError(mode, category, code, storageAdapter) {
    ...
    var errors = readJSON(ERRORS_KEY, [], isErrorsArray, storageAdapter);
    errors.push({ date: localDateString(), mode: mode, category: category, code: code });
    var trimmed = errors.length > MAX_ERROR_ENTRIES ? errors.slice(-MAX_ERROR_ENTRIES) : errors;
  ...
  function resetDiagnostics(storageAdapter) {
    DIAGNOSTICS_KEYS.forEach(function (key) {
      removeRaw(key, storageAdapter);
    });
  }
  ...
  var api = {
    ...
    recordLocalReturn: recordLocalReturn,
    computeSevenDayRetention: computeSevenDayRetention,
  ...
  if (typeof window !== 'undefined') {
    ...
    window.DinoQuiz.services.diagnostics = api;
  }
})();
