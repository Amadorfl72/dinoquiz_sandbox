...
describe('"Volver a jugar" button style meets 64dp height / 48dp width / 24sp text (AC-2/AC-23)', () => {
  ...
  test('the base and play-again-specific CSS rules together satisfy the minimums', () => {
    ...
    // themselves (TRIOFSND-133) — resolve `var(--x)` against that :root map
    // before asserting, same as the Home screen's equivalent check.
    ...
    const minHeight = Math.max(
      ...Array.from(combinedRule.matchAll(/min-height:\s*([^;]+);/g)).map((match) =>
        parseFloat(resolve(match[1].trim()))
      )
    );
    const minWidth = Math.max(
      ...Array.from(combinedRule.matchAll(/min-width:\s*([^;]+);/g)).map((match) =>
        parseFloat(resolve(match[1].trim()))
      )
    );
    const fontSizePx = Math.max(
      ...Array.from(combinedRule.matchAll(/font-size:\s*([^;]+);/g)).map((match) =>
        parseFloat(resolve(match[1].trim()))
      )
    );
