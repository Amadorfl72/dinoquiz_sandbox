...
describe('QuestionScreen', () => {
  ...
  test('the score text style meets the minimum 20sp font size (TRIOFSND-83)', () => {
    ...
    // property (`var(--font-size-body)`); resolve it against :root so this
    // still reads the effective px value instead of the var() call.
    ...
    const rule = ruleMatch[1].replace(/var\(\s*--([\w-]+)\s*\)/g, (whole, name) => rootVars[name] || whole);
