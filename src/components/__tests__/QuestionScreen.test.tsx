...
describe('TRIOFSND-19: Implement Incorrect Answer Feedback', () => {
  ...
  it('highlights the correct option and shows a gentle non-punitive message on incorrect tap', () => {
    ...
    expect(correctOption.props.style).toEqual(
      expect.arrayContaining([expect.objectContaining({ backgroundColor: '#4CAF50' })]) // Assuming green highlight
    );

    // Check non-punitive message is displayed
    expect(getByText(/Don't worry, you'll get the next one!|Oops!|Keep trying/i)).toBeTruthy();
  });
