 ...
 * Spying on those renderer functions -- never reading a DOM class after the
 * fact, and never reading back `dinoquiz:lastMode` (written by the
 * selector's own click handler the instant a card is tapped, before
 * dispatch decides which engine to start, so it reflects the tap, not the
 * start) -- proves which engine main.js actually invoked for a given tap.
 * `OWN_SCREEN_SELECTORS` is kept as a second, independent, user-facing
 * signal: the tapped mode's dedicated screen must be the one visible on
 * screen, and no other mode's screen (`.question-screen` included) may be
 * present.
 *
 * Out of scope here (TRIOFSND-294, a separate ticket/branch): wiring
 * `lineaDelTiempo`'s own dispatch case and engine in main.js. This suite
 * only asserts against main.js as it stands today -- it is expected, and
 * required, to fail red for `lineaDelTiempo` until that wiring ships;
 * turning it green is that ticket's job, not this one's.
 */
...
/** Taps a mode selector card by its real accessible label, the same real interaction a player performs. */
...
describe('despacho derivado de modesCatalog.js: pulsar cada modo inicia SU propio motor, nunca el quiz silencioso', () => {
  let container;
  let hadOwnFetch;
  let originalFetch;
  ...
  modesCatalog.MODES_CATALOG.forEach((mode) => {
    test(`despacho "${mode.id}": el motor que arranca es el suyo propio, nunca otro`, async () => {
      ...
      // main.js actually invoked, not a DOM class or `dinoquiz:lastMode`
      // read after the fact -- must equal the id tapped, for every catalog
      // mode, and no other mode's engine may fire alongside it.
