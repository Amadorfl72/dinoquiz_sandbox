...
describe('TRIOFSND-100/TRIOFSND-84: app-shell navigation Quiz -> Resultados -> Volver a jugar / Salir', () => {
  ...
  afterEach(() => {
    // A test that doesn't walk a game all the way to completion can leave a
    // stale auto-advance `setTimeout` (public/scripts/main.js) queued on the
    // fake-timer clock; without clearing it here it can fire later, against
    // a different test's container, once fake timers are re-enabled.
    jest.clearAllTimers();
    jest.useRealTimers();
    container.remove();
    jest.useRealTimers();
  ...
  test('"Volver a jugar" resets game state and navigates to the first question of a new game', async () => {
    ...
    try {
      const { resolveScreenRenderers, startNewGame } = require(MAIN_JS_PATH);
      const renderers = resolveScreenRenderers();
      // More than QUESTIONS_PER_GAME so the replay's anti-repeat check
      // (TRIOFSND-99/TRIOFSND-102) has another valid 10-question set to pick.
  ...
  describe('avance automático tras el temporizador (TRIOFSND-84)', () => {
    ...
    test('auto-advances straight to Resultados when the last question times out unanswered-via-"Siguiente"', async () => {
      const { resolveScreenRenderers, startNewGame, AUTO_ADVANCE_GRACE_MS } = require(MAIN_JS_PATH);
      const renderers = resolveScreenRenderers();
      const questions = buildQuestionBank(10);
      ...
      // A partida always has exactly 10 questions (TRIOFSND-99), so walk the
      // first 9 manually via "Siguiente" and only let the 10th (last) one
      // time out, to exercise the auto-advance-to-Resultados path itself.
      for (let i = 0; i < 9; i += 1) {
        await answerCurrentQuestion(container, { correct: true });
      }
  ...
  test('TRIOFSND-92: an incorrect answer records the aggregated, non-PII pregunta_respondida and pregunta_respondida_fallo events', () => {
    ...
    return renderHome(document, renderers.renderHomeScreen, fetchFn, storage).then(async () => {
      getByRole(container, 'button', { name: require('../../public/i18n/es.json').home.playButton }).click();
...
describe('TRIOFSND-95: end of game (pregunta 10) computes score and racha, then navigates to Resultados', () => {
  ...
  async function playGameWithPattern(pattern) {
    ...
    try {
      ...
      // See the top-level describe's afterEach: clear any stale queued
      // auto-advance timer before handing the fake-timer clock back.
      jest.clearAllTimers();
      jest.useRealTimers();
    }
...
describe('TRIOFSND-97: Resultados banner/rewarded ad gated by the remove-ads purchase flag', () => {
  ...
  test('a purchase confirmed on Home hides ads on the very next game\'s Resultados screen', async () => {
    ...
    await renderHome(document, renderers.renderHomeScreen, fetchFn, undefined, undefined, storageObj);
...
describe('TRIOFSND-102: aggregated partida_iniciada / replay_pulsado local counters', () => {
  let container;

  beforeEach(() => {
    // A preceding test elsewhere in this file can leave a stale queued
    // auto-advance `setTimeout` on the (Jest-process-wide) fake-timer clock;
    // start every test in this describe from a clean, real-timer baseline
  ...
  // A real DinoQuizStorage backed by a plain Map "backend" shared across
  // instances -- the same aggregated-analytics store the app uses
  // (analyticsEventCounts under the existing `dinoquiz:` root), not a
  // second implementation. Sharing `backingStore` across instances is what
  // lets a test simulate a reload via `new DinoQuizStorage(...)`.
  function createSharedAnalyticsStorage(backingStore) {
    const { DinoQuizStorage } = require('../../src/services/storage/StorageClient');
    const adapter = {
      name: 'memory',
      isAvailable: async () => true,
      getItem: async (key) => (backingStore.has(key) ? backingStore.get(key) : null),
    ...
    return new DinoQuizStorage([adapter]);
  }

  /** Walks a full 10-question game (all hits), collecting each question's prompt text along the way. */
  async function playFullGameCollectingPrompts() {
    const prompts = [];
    for (let i = 0; i < 10; i += 1) {
      prompts.push(container.querySelector('.question-screen__prompt').textContent);
      await answerCurrentQuestion(container, { correct: true });
    }
    return prompts;
  }

  test('¡Jugar! (a direct game start) increments partida_iniciada exactly once and never touches replay_pulsado', async () => {
    const backingStore = new Map();
    const { resolveScreenRenderers, startNewGame } = require(MAIN_JS_PATH);
    const renderers = resolveScreenRenderers();
    const questions = buildQuestionBank(20);
    const storage = createSharedAnalyticsStorage(backingStore);

    const session = startNewGame(container, renderers, questions, document, undefined, () => 0, storage, storage);
    // recordEvent() is fire-and-forget from main.js's point of view; flush
    // the microtask queue (a real macrotask boundary) before reading it back.
    await flushPromises();
  ...
  test('a correct replay increments replay_pulsado by 1 and partida_iniciada by 1, with a distinct 10-question set', async () => {
    const backingStore = new Map();
    const { resolveScreenRenderers, startNewGame } = require(MAIN_JS_PATH);
    const renderers = resolveScreenRenderers();
    const questions = buildQuestionBank(20); // > QUESTIONS_PER_GAME, so a different set is achievable
    const storage = createSharedAnalyticsStorage(backingStore);

    startNewGame(container, renderers, questions, document, undefined, () => 0, storage, storage);
    await flushPromises();

    let firstGamePrompts;
    let secondGamePrompts;
    jest.useFakeTimers();
    try {
      firstGamePrompts = await playFullGameCollectingPrompts();
      await flushPromises();
      ...
      expect(await storage.getEventCount('replay_pulsado')).toBe(1);
      expect(await storage.getEventCount('partida_iniciada')).toBe(2);
      expect(container.querySelector('.question-screen')).not.toBeNull();

      secondGamePrompts = await playFullGameCollectingPrompts();
    } finally {
      jest.clearAllTimers();
      jest.useRealTimers();
    }

    // Exactly 10 unique questions in both the original game and the replay
    // (TRIOFSND-99), and the replay's set differs from the previous one,
    // order-independently (they're compared as sets here).
    expect(new Set(firstGamePrompts).size).toBe(10);
    expect(new Set(secondGamePrompts).size).toBe(10);
    expect(new Set(secondGamePrompts)).not.toEqual(new Set(firstGamePrompts));
  });

  test('a replay whose new partida fails validation keeps the replay_pulsado increment but does not increment partida_iniciada', async () => {
    const backingStore = new Map();
    const { resolveScreenRenderers, startNewGame } = require(MAIN_JS_PATH);
    const renderers = resolveScreenRenderers();
    // Exactly QUESTIONS_PER_GAME questions: any replay necessarily reselects
    // the very same set (only reordered), so gameFlow.startNewGame rejects it.
    const questions = buildQuestionBank(10);
    const storage = createSharedAnalyticsStorage(backingStore);
    ...
    jest.useFakeTimers();
    try {
      await playFullGameCollectingPrompts();
      await flushPromises();
      ...
      expect(await storage.getEventCount('replay_pulsado')).toBe(1);
      expect(await storage.getEventCount('partida_iniciada')).toBe(1); // unchanged: the replay was rejected
      // The failed replay never left Resultados -- no new partida was presented.
      expect(container.querySelector('.results-screen')).not.toBeNull();
  ...
  test('re-rendering/re-mounting Resultados does not change any counter on its own', async () => {
    const backingStore = new Map();
    const { resolveScreenRenderers, startNewGame } = require(MAIN_JS_PATH);
    const renderers = resolveScreenRenderers();
    const questions = buildQuestionBank(20);
    ...
    // Simulates navigating away and back, or a reload restoring the same
    // finished-game screen -- mounting/rendering Resultados by itself must
    // never touch the counters (only an accepted "Volver a jugar" click does).
    renderers.renderResultsScreen(container, { score: 10, adsRemoved: true });
    renderers.renderResultsScreen(container, { score: 10, adsRemoved: true });
  ...
  test('duplicate signals from a single accepted "Volver a jugar" activation do not double-count', async () => {
    const backingStore = new Map();
    const { resolveScreenRenderers, startNewGame } = require(MAIN_JS_PATH);
    const renderers = resolveScreenRenderers();
    const questions = buildQuestionBank(20);
    ...
    try {
      ...
      // Two signals for the very same accepted activation (e.g. a stray
      // duplicate event/callback) -- only the first one should count.
  ...
  test('two independent, later "Volver a jugar" activations each count as their own replay', async () => {
    const backingStore = new Map();
    const { resolveScreenRenderers, startNewGame } = require(MAIN_JS_PATH);
    const renderers = resolveScreenRenderers();
    const questions = buildQuestionBank(30);
    const storage = createSharedAnalyticsStorage(backingStore);
    ...
    try {
      ...
      // The interface admits a fresh, independent activation once the
      // replayed game itself reaches a new Resultados screen.
      getByRole(container, 'button', { name: strings.playAgainButton }).click();
      await flushPromises();
      expect(await storage.getEventCount('replay_pulsado')).toBe(2);
  ...
  test('accumulates on top of an existing total already greater than 0 (e.g. from a previous session)', async () => {
    const backingStore = new Map();
    const seedStorage = createSharedAnalyticsStorage(backingStore);
    await seedStorage.set('analyticsEventCounts', { partida_iniciada: 4, replay_pulsado: 2 });
  ...
  test('counters survive re-requiring main.js against the same backing storage (module reset / reload)', async () => {
    const backingStore = new Map();

    const firstRun = require(MAIN_JS_PATH);
    const questions = buildQuestionBank(20);
    const firstStorage = createSharedAnalyticsStorage(backingStore);
    firstRun.startNewGame(
      container,
      firstRun.resolveScreenRenderers(),
      questions,
      document,
      undefined,
      () => 0,
    ...
    await flushPromises();
    expect(await firstStorage.getEventCount('partida_iniciada')).toBe(1);

    // Simulates an app reload: a fresh main.js module instance, and a fresh
    // DinoQuizStorage instance reading the same underlying backend.
    jest.resetModules();
    const secondRun = require(MAIN_JS_PATH);
    const secondStorage = createSharedAnalyticsStorage(backingStore);

    expect(await secondStorage.getEventCount('partida_iniciada')).toBe(1);

    secondRun.startNewGame(
      container,
      secondRun.resolveScreenRenderers(),
      questions,
      document,
      undefined,
      () => 0,
    ...
    expect(await secondStorage.getEventCount('partida_iniciada')).toBe(2);
  });
});
